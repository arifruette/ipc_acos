...
void send_fifo(const string &msg) {
    // try to write to pre-opened fifo_fd if available
    if (fifo_fd >= 0) {
        ssize_t w = write(fifo_fd, msg.c_str(), msg.size());
        if (w == (ssize_t)msg.size()) return;
        // if write failed, we'll fallback to open/close below
    }
    // try open-while-write (one-shot) — don't block if no reader
...
void handle_sigint(int) {
    ...
    notify_all_students();
    // wake up teacher if waiting on queue_sem
...
int main(int argc, char *argv[]) {
    ...
    // ensure FIFO exists; observer may also create it, mkfifo is idempotent
    if (mkfifo(FIFO_NAME, 0666) == -1 && errno != EEXIST) {
        perror("mkfifo");
        // nonfatal — still continue, we'll fallback to one-shot open
    } else {
        // try to open FIFO for long-living writes; non-blocking so it won't hang if no reader
        fifo_fd = open(FIFO_NAME, O_WRONLY | O_NONBLOCK);
        if (fifo_fd < 0) {
            // no reader yet; that's fine — we'll use one-shot opens later
            fifo_fd = -1;
        }
    }

    // create shared memory of required size
    ...
    shm = (SharedData*) mmap(nullptr, shm_size, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
    if (shm == MAP_FAILED) {
        perror("mmap");
        close(shm_fd);
        shm_unlink(SHM_NAME);
    ...
    sem_unlink(MUTEX_NAME); // remove leftover if any
    ...
    while (running) {
        ...
        // wait for student's ack, but handle EINTR
        while (true) {
            if (sem_wait(ack) == -1) {
                if (errno == EINTR) {
                    if (!running) break;
