...
void cleanup() {
    ...
    if (shm) { munmap(shm, sizeof(SharedData)); shm = nullptr; } // student mapped full size earlier
...
int main() {
    ...
    shm = (SharedData*) mmap(nullptr, map_size, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
    if (shm == MAP_FAILED) {
        perror("mmap");
        close(shm_fd);
        return 1;
