...
# **5. Реализация на 10 баллов — несколько наблюдателей**

Для четвёртой программы в дополнение к реализации на 9 баллов была добавлена возможность одновременного запуска нескольких наблюдателей.

Все наблюдатели:

* являются независимыми процессами;
* запускаются в отдельных консолях;
* используют один и тот же именованный канал FIFO `/tmp/exam_log`;
* работают по одинаковому коду (`observer.cpp`);
* корректно завершаются по `SIGINT` (Ctrl+C), не нарушая работу других процессов.

Технически это реализовано за счёт свойств именованного канала POSIX:

* FIFO `/tmp/exam_log` создаётся (при необходимости) при запуске каждого наблюдателя или преподавателя;
* несколько процессов могут одновременно открыть один и тот же FIFO на чтение;
* процессы `teacher` и `student` пишут сообщения в FIFO, а все запущенные наблюдатели читают и выводят их в свои консоли.

В `observer.cpp` дополнительно выводится PID наблюдателя, что позволяет визуально отличать несколько одновременно работающих наблюдателей.

## Тестирование

![10 test png](test/10.png)

## Запуск

- Переходим в каталог с реализацией на 10 баллов:
```bash
cd 10
````
...
Дальше все процессы запускаются **независимо друг от друга** в разных консолях.

1. **Запускаем одного или нескольких наблюдателей** (каждый в своей консоли):
...
2. **Запускаем преподавателя** (в отдельном терминале):
...
3. **Запускаем студентов**:

Один студент (в отдельной консоли):

```bash
./student
```

Много студентов через скрипт:

```bash
./run_students.sh <количество_студентов>
```

Таким образом, в дополнение к реализации на 9 баллов реализована программа, допускающая подключение нескольких наблюдателей, каждый из которых выводит информацию в свою консоль, при этом все наблюдатели работают одинаково и согласованно.
...
4. На уровне **10 баллов** была добавлена возможность одновременного подключения нескольких наблюдателей:
  * все наблюдатели используют один и тот же именованный FIFO `/tmp/exam_log`;
  * работают по одному и тому же коду `observer.cpp`;
  * каждый выводит получаемую информацию в свою консоль;
  * корректно сосуществуют друг с другом и с основными процессами (`teacher`, `student`).
